...
// Modified by Sergey Grigorev: http://ssdc.getdev.tk
using System;
...
namespace Game
{
    ...
    {
        bool lookat;
        float applysmooth = 2.0f, applyup = .15f;
        ...
        [Config("CarCamera", "carCameraType")]
        static protected CarCameraType carCameraType;

        [Config("Camera", "tpsCameraDistance")]
        static float tpsCameraDistance = 4;
        [Config("Camera", "tpsCameraCenterOffset")]
        static Vec3 tpsCameraCenterOffset = new Vec3(0, 0, 1.6f);

        [Config("Camera", "tpsVehicleCameraDistance")]
        static float tpsVehicleCameraDistance = 3.4f;
        [Config("Camera", "tpsVehicleCameraCenterOffset")]
        static Vec3 tpsVehicleCameraCenterOffset = new Vec3(0, 0, 0.45f);
        /*in car view: tpsVehicleCameraDistance = 0.05 tpsVehicleCameraCenterOffset = 0.4*/
        ...
        protected override bool OnKeyDown(KeyEvent e)
        {
            ...
            if (e.Key == EKeys.C)
            {
                /*if (lookat) { lookat = false; } else lookat = true;
                return true;*/
                carCameraType = (CarCameraType)((int)carCameraType + 1);
                if (carCameraType == CarCameraType.Count)
                    carCameraType = (CarCameraType)0;
                GameEngineApp.Instance.AddScreenMessage("Car camera type: " + carCameraType.ToString());
                return true;
            }
        ...
        protected override void OnTick(float delta)
        {
            ...
            if (lookat)
            {
                SmoothLookAtCar(delta, applysmooth, applyup);//2.0f,.15f
            }
            ...
            if (GetRealCameraType() == CameraType.TPS && !IsCutSceneEnabled() && !activeConsole)
            {
                ...
                if (IsPlayerUnitVehicle())
                {
                    ...
                    cameraCenterOffset = tpsVehicleCameraCenterOffset.Z;
                }
                else
                {
                    cameraDistance = tpsCameraDistance;
                    cameraCenterOffset = tpsCameraCenterOffset.Z;
                }
                ...
                if (IsPlayerUnitVehicle())
                {
                    ...
                    tpsVehicleCameraCenterOffset = new Vec3(tpsVehicleCameraCenterOffset.X,
                        tpsVehicleCameraCenterOffset.Y, cameraCenterOffset);
                }
                else
                {
                    tpsCameraDistance = cameraDistance;
                    tpsCameraCenterOffset = new Vec3(tpsCameraCenterOffset.X,
                        tpsCameraCenterOffset.Y, cameraCenterOffset);
                }
            }
        ...
        void SmoothLookAtCar(float delta, float smooth, float up)
        {
            Unit car = GetPlayerUnit() as Car;
            if (car == null) return;
            SphereDir pdir = PlayerIntellect.Instance.LookDirection;
            SphereDir cdir = SphereDir.FromVector(car.Rotation.GetForward());
            Vec3 newup = new Vec3(cdir.GetVector().X, cdir.GetVector().Y, cdir.GetVector().Z - up);
            SphereDir newdir = SphereDir.FromVector(Vec3.Lerp(pdir.GetVector(), newup, smooth * delta));
            PlayerIntellect.Instance.LookDirection = newdir;
        }
        ...
        void UpdateHUD()
        {
            ...
            if (car != null)
            {
                hudControl.Controls["Game"].Visible = false;
            }
            else
            {
                ...
                case CameraType.TPS:
                    {
                        ...
                        float cameraDistance;
                        Vec3 cameraCenterOffset;

                        if (IsPlayerUnitVehicle() & carCameraType == CarCameraType.Back)
                        {
                            lookat = true;
                            applysmooth = 2.0f; applyup = .15f;
                            cameraDistance = tpsVehicleCameraDistance;
                            cameraCenterOffset = tpsVehicleCameraCenterOffset;
                        }
                        else if (IsPlayerUnitVehicle() & carCameraType == CarCameraType.In)
                        {
                            lookat = true;
                            applysmooth = 8.0f; applyup = .05f;
                            cameraDistance = 0.05f;
                            cameraCenterOffset = new Vec3(0.3f, 0.4f, 0.35f) * unit.GetInterpolatedRotation();
                        }
                        else if (IsPlayerUnitVehicle() & carCameraType == CarCameraType.Left)
                        {
                            lookat = true;
                            applysmooth = 8.0f; applyup = .05f;
                            cameraDistance = 1.6f;
                            cameraCenterOffset = new Vec3(0, 1.0f, 0.6f) * unit.GetInterpolatedRotation();
                        }
                        else if (IsPlayerUnitVehicle() & carCameraType == CarCameraType.Free)
                        {
                            lookat = false;
                            cameraDistance = tpsVehicleCameraDistance;
                            cameraCenterOffset = tpsVehicleCameraCenterOffset;
                        ...
                        Vec3 lookAt = unit.GetInterpolatedPosition() + cameraCenterOffset;
